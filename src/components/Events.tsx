...
import { Stack, Text, Icon, Link, useTheme, mergeStyleSets, IStackTokens, ITheme } from '@fluentui/react';
...
const eventDetailsTokens: IStackTokens = { childrenGap: 6 };
const metaTokens: IStackTokens = { childrenGap: 4 };
...
export default function Events() {
  ...
  useEffect(() => {
    supabase.from('events').select('*').order('event_date', { ascending: true }).limit(2).then(({ data }) => {
  ...
  return (
    <Stack tokens={{ childrenGap: 20 }} className={styles.container}>
      <Stack horizontal horizontalAlign="space-between" verticalAlign="center">
        ...
        >
          {t('events.viewCalendar')} <Icon iconName="ChevronRight" styles={{ root: { fontSize: '12px' } }} />
        </Link>
      </Stack>
            ...
            return (
              ...
              >
                ...
                <Stack horizontal tokens={eventHeaderTokens} verticalAlign="start">
                  ...
                  >
                    <Text styles={{ root: { fontSize: '22px', fontWeight: 700, color: theme.palette.white } }}>
                      {dateInfo.day}
                    </Text>
                    <Text styles={{ root: { fontSize: '11px', fontWeight: 600, opacity: 0.9, marginTop: '4px', color: theme.palette.white } }}>
                      {dateInfo.month}
                    </Text>
                  </div>
                  <Stack tokens={eventDetailsTokens} styles={{ root: { flex: 1 } }}>
                    ...
                    <Stack tokens={metaTokens}>
                      <Stack horizontal tokens={{ childrenGap: 4 }} verticalAlign="center">
                        <Icon iconName="Clock" styles={{ root: { fontSize: 12, color: theme.palette.neutralSecondary } }} />
                        <Text variant="small" styles={{ root: { color: theme.palette.neutralSecondary, fontSize: '11px' } }}>
                          {dateInfo.time}
                        </Text>
                      </Stack>
                      <Stack horizontal tokens={{ childrenGap: 4 }} verticalAlign="center">
                        <Icon iconName="POI" styles={{ root: { fontSize: 12, color: theme.palette.neutralSecondary } }} />
                        <Text variant="small" styles={{ root: { color: theme.palette.neutralSecondary, fontSize: '11px' } }}>
                          {event.location}
                        </Text>
                      </Stack>
                ...
                <Stack
                  ...
                  styles={{ root: { borderTop: `1px solid ${theme.palette.neutralLighter}` } }}
                >
                  {!isDummy ? (
                    <Text variant="small" styles={{ root: { color: theme.palette.themePrimary, fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' } }}>
                      {t('events.learnMore')} <Icon iconName="ChevronRight" styles={{ root: { fontSize: '10px' } }} />
                    </Text>
                  ) : (
                    <Text variant="small" styles={{ root: { color: theme.palette.neutralTertiary, fontWeight: 500, fontStyle: 'italic' } }}>
                      Coming soon
                    </Text>
                  )}
                </Stack>
